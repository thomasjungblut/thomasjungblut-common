...
import static de.jungblut.math.matrix.StrassenMatrixMultiplication.add;
...
public class ForkJoinStrassenMatrixMultiplication {
	...
	public static final class StrassenComputer extends
		...
		protected double[][] compute() {
			...
			ForkJoinTask<double[][]> fork1 = new StrassenComputer(a, add(f, h,
					-1)).fork(); // P1 = a(f-h) = af-ah
			ForkJoinTask<double[][]> fork2 = new StrassenComputer(add(a, b, 1),
					h).fork(); // P2 = (a+b)h = ah+bh
			ForkJoinTask<double[][]> fork3 = new StrassenComputer(add(c, d, 1),
					e).fork();// P3 = (c+d)e = ce+de
			ForkJoinTask<double[][]> fork4 = new StrassenComputer(d, add(g, e,
					-1)).fork();// P4 = d(g-e) = dg-de
			// P5 = (a+d)(e+h)=ae+de+ah+dh
			ForkJoinTask<double[][]> fork5 = new StrassenComputer(add(a, d, 1),
					add(e, h, 1)).fork();
			// P6 = (b-d)(g+h)=bg-dg+bh-dh
			ForkJoinTask<double[][]> fork6 = new StrassenComputer(
					add(b, d, -1), add(g, h, 1)).fork();
			// P7 = (a-c)(e+f)=ae-ce+af-cf
			ForkJoinTask<double[][]> fork7 = new StrassenComputer(
					add(a, c, -1), add(e, f, 1)).fork();
