...
public final class DenseDoubleMatrix {
	...
	/**
	 * Get a whole column of the matrix as vector. If the specified column
	 * doesn't exist a IllegalArgumentException is thrown.
	 * 
	 * @param col
	 * @return
	...
	public final Tuple<DenseDoubleMatrix, DenseDoubleVector> splitLastColumn() {
		DenseDoubleMatrix m = new DenseDoubleMatrix(getRowCount(), getColumnCount() - 1);
	...
	/**
	 * Creates two matrices out of this by the given percentage. It uses a
	 * random function to determine which rows should belong to the matrix
	 * including the given percentage amount of rows.
	 * 
	 * @param percentage
	 ...
	 */
	public final Tuple<DenseDoubleMatrix, DenseDoubleMatrix> splitRandomMatrices(float percentage) {
		if (percentage < 0.0f || percentage > 1.0f) {
			throw new IllegalArgumentException("Percentage must be between 0.0 and 1.0! Given "
					+ percentage);
		}
		...
		return new Tuple<DenseDoubleMatrix, DenseDoubleMatrix>(new DenseDoubleMatrix(firstMatrix),
				new DenseDoubleMatrix(secondMatrix));
	}
	...
	public final DenseDoubleMatrix multiply(DenseDoubleMatrix other) {
		if (JCUDAMatrixUtils.CUDA_AVAILABLE) {
			...
			if (this.numColumns != other.getRowCount()) {
				throw new IllegalArgumentException("multiply: nonconformant arguments (this is "
						+ this.numRows + "x" + this.numColumns + ", other is "
						+ other.getRowCount() + "x" + other.getColumnCount() + ")");
			}

			DenseDoubleMatrix matrix = new DenseDoubleMatrix(this.numRows, other.numColumns);
	...
	public final DenseDoubleMatrix multiplyElementWise(DenseBooleanMatrix other) {
		DenseDoubleMatrix matrix = new DenseDoubleMatrix(this.numRows, this.numColumns);
	...
	public DenseDoubleMatrix slice(int rowOffset, int rowMax, int colOffset, int colMax) {
		DenseDoubleMatrix m = new DenseDoubleMatrix(rowMax - rowOffset, colMax - colOffset);
	...
	public static DenseDoubleMatrix copy(DenseDoubleMatrix matrix) {
		...
		final double[][] dest = new double[matrix.getRowCount()][matrix.getColumnCount()];
	...
	public static DenseDoubleMatrix multiplyTransposedVectors(DenseDoubleVector transposed,
			DenseDoubleVector normal) {
		DenseDoubleMatrix m = new DenseDoubleMatrix(transposed.getLength(), normal.getLength());
