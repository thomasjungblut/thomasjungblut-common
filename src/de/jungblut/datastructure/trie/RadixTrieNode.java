...
public final class RadixTrieNode<T> implements Comparable<RadixTrieNode<T>> {
   ...
   * value is null. This is just a concat of the paths which leads to this node.
   * Used for wildcard matching to get the distance between an input and this
   * leaf.
   */
  ...
  @SafeVarargs
  public static final <T> RadixTrieNode<T> createTrieNode(String key,
      String value, T... documents) {
  ...
  public static final <T> RadixTrieNode<T> createTrieNode(String key,
      String value, ArrayList<T> documents) {
    RadixTrieNode<T> node = new RadixTrieNode<T>();
    node.key = key.intern();
    node.value = value;
    node.doc = documents;
