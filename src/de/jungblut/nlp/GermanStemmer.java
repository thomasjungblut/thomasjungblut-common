 ...
 * </p>
 */
public final class GermanStemmer {
  /**
   ...
   * discriminators in the most of those cases. The algorithm is context free,
  ...
  private void optimize(StringBuilder buffer) {
    ...
    if (buffer.length() > 5
        && buffer.substring(buffer.length() - 5, buffer.length()).equals(
            "erin*")) {
      buffer.deleteCharAt(buffer.length() - 1);
      strip(buffer);
    }
    // Additional step for irregular plural nouns like "Matrizen -> Matrix".
    // NOTE: this length constraint is probably not a great value, its just to
