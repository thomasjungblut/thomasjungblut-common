 ...
 */
public class SupervisedDiscretize extends Filter implements SupervisedFilter,
        OptionHandler, WeightedInstancesHandler, TechnicalInformationHandler {

    private int label_size = 32;
     ...
     */
    private final Range m_DiscretizeCols = new Range();

    /**
     * Store the current cutpoints
     */
    private double[][] m_CutPoints = null;

    /**
     * Output binary attributes for discretized attributes.
     */
    private boolean m_MakeBinary = false;

    /**
     * Use better encoding of split point for MDL.
     */
    private boolean m_UseBetterEncoding = false;

    /**
     * Use Kononenko's MDL criterion instead of Fayyad et al.'s
     */
    private boolean m_UseKononenko = false;

    /**
     * Constructor - initialises the filter
     */
    public SupervisedDiscretize() {
    ...
    public Enumeration listOptions() {
        ...
        newVector.addElement(new Option(
                "\tOutput binary attributes for discretized attributes.", "D",
                0, "-D"));

        newVector.addElement(new Option(
                "\tUse better encoding of split point for MDL.", "E", 0, "-E"));

        newVector.addElement(new Option("\tUse Kononenko's MDL criterion.",
                "K", 0, "-K"));
     ...
     *
     * @param rangeList a string representing the list of attributes. Since the string
     ...
     * among the selection will be Discretized).
     *
     * @param attributes an array containing indexes of attributes to Discretize. Since
     ...
     */
    void calculateCutPointsByMDL(int index, Instances data) {
    ...
    private boolean KononenkosMDL(double[] priorCounts, double[][] bestCounts,
        ...
        distPrior = SpecialFunctions.log2Binomial(numInstances
                + numClassesTotal - 1, numClassesTotal - 1);
        ...
        for (double[] bestCount : bestCounts) {
            ...
            distAfter += SpecialFunctions.log2Binomial(sum + numClassesTotal
                    - 1, numClassesTotal - 1);
    ...
    private boolean FayyadAndIranisMDL(double[] priorCounts,
        ...
        // Check if split is to be accepted
        return (gain > (Utils.log2(numCutPoints) + delta)
                / numInstances);
    }
    ...
    private double[] cutPointsForSubset(Instances instances, int attIndex,
        ...
        double currentCutPoint, bestCutPoint = -1, currentEntropy, bestEntropy, priorEntropy, gain;
        ...
        for (int i = first; i < (lastPlusOne - 1); i++) {
            ...
            counts[1][(int) instances.instance(i).classValue()] -= instances
                    .instance(i).weight();
            if (instances.instance(i).value(attIndex) < instances.instance(
                    i + 1).value(attIndex)) {
                currentCutPoint = (instances.instance(i).value(attIndex) + instances
                        .instance(i + 1).value(attIndex)) / 2.0;
                currentEntropy = ContingencyTables
                        .entropyConditionedOnRows(counts);
        ...
        if ((m_UseKononenko && KononenkosMDL(priorCounts, bestCounts,
                ...
                || (!m_UseKononenko && FayyadAndIranisMDL(priorCounts,
                bestCounts, numInstances, numCutPoints))) {
            ...
            if ((left == null) && (right) == null) {
                ...
                System.arraycopy(left, 0, cutPoints, 0, left.length);
                cutPoints[left.length] = bestCutPoint;
                System.arraycopy(right, 0, cutPoints, left.length + 1,
                        right.length);
            }
    ...
    void setOutputFormat() {
        ...
        ArrayList attributes = new ArrayList(getInputFormat().numAttributes());
        int classIndex = getInputFormat().classIndex();
        for (int i = 0; i < getInputFormat().numAttributes(); i++) {
            if ((m_DiscretizeCols.isInRange(i))
                    && (getInputFormat().attribute(i).isNumeric())) {
                if (!m_MakeBinary) {
                    ArrayList attribValues = new ArrayList(1);
                    if (m_CutPoints[i] == null) {
                        attribValues.add("'All'");
                    } else {
                        for (int j = 0; j <= m_CutPoints[i].length; j++) {
                            if (j == 0) {
                                attribValues.add("'(-inf-"
                                        + Utils.doubleToString(
                                        m_CutPoints[i][j], label_size)
                                        + "]'");
                    ...
                    attributes.add(new Attribute(getInputFormat()
                            .attribute(i).name(), attribValues));
                } else {
                    if (m_CutPoints[i] == null) {
                        ArrayList attribValues = new ArrayList(1);
                        attribValues.add("'All'");
                        ...
                        for (int j = 0; j < m_CutPoints[i].length; j++) {
                            ArrayList attribValues = new ArrayList(2);
                            attribValues.add("'(-inf-"
                                    + Utils.doubleToString(m_CutPoints[i][j],
                                    label_size) + "]'");
                            attribValues.add("'("
                                    + Utils.doubleToString(m_CutPoints[i][j],
                                    label_size) + "-inf)'");
                            attributes.add(new Attribute(
                                    getInputFormat().attribute(i).name() + "_"
                                            + (j + 1), attribValues));
                        }
                    }
                }
            } else {
                attributes.add(getInputFormat().attribute(i).copy());
            }
        }
     ...
     */
    void convertInstance(Instance instance) {
    ...
    public static void main(String[] argv) {
        runFilter(new SupervisedDiscretize(), argv);
    }
}
